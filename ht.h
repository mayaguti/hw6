...
public:
    ...
    HASH_INDEX_T next() 
    {
		...
		HASH_INDEX_T nextLoc = (this->start_ + dhstep_*this->numProbes_) % this->m_;
		this->numProbes_++;
		return nextLoc;
    }
};
...
{
    if((double)(nonDeleted_+deleted_)/CAPACITIES[mIndex_] >= maxLoadingFactor_){
        resize();
    }
    HASH_INDEX_T insertLoc_ = this->probe(p.first);
    if(npos == insertLoc_) {
        throw std::logic_error("No available location");
    }
    if(table_[insertLoc_] != nullptr){ 
        table_[insertLoc_]->item.second = p.second;
    }
    else {
        table_[insertLoc_] = new HashItem(p);
        nonDeleted_++;
    }
}
...
void HashTable<K,V,Prober,Hash,KEqual>::resize()
{
    ...
    mIndex_++;
    std::vector<HashItem*> newTable_(CAPACITIES[mIndex_], nullptr);
    ...
    for(HASH_INDEX_T i=0; i < CAPACITIES[mIndex_-1]; i++){
        if(oldTable_[i] != nullptr){
            if(oldTable_[i]->deleted){
                ...
                table_[this->probe(oldTable_[i]->item.first)] = oldTable_[i];
                nonDeleted_++;
            }
        }
    }
